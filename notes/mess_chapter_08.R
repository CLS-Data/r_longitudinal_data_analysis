...
unique(df_ncds$fup) %>%
  map_dfr(run_regression) %>%
  arrange(fup)

mod_covars <- lst(
  basic = c("sex", "ethnic_group"),
  parent_bmi = c(basic, "father_bmi", "mother_bmi"),
  parent_edu = c(basic, "father_education", "mother_education"),
...
expand_grid(
  ...
  cog_var = str_subset(names(df_ncds), "^(verbal|vocab)"),
  model = names(mod_covars)
)

run_regression <- function(fup, cog_var, model){
  df <- df_ncds %>%
    filter(fup == !!fup) %>%
    select(bmi, cog_var = all_of(!!cog_var),
           all_of(mod_covars[[!!model]])) %>%
    drop_na()
}

# Task 1  - Amend code for MCS and add in BMI z-scores
#         - Amend code so that MCS and NCDS can be called from both
#         - Amend code for attrition from previous chapter
