...
Again, data are in Stata (`.dta`) format, so we will load them using the `read_dta()` function from `haven` package. Let's load the `tidyverse` and `haven` packages, as well as `codebook`, `labelled`, `summarytools` and `glue`, the latter of which we'll use to construct file paths. Let's also load the cohort member derived variable dataset from Sweep 7 (17y) of the MCS:
...
1. Create a codebook for the NCDS Sweep 9 (55y) derived variable dataset (`55y/ncds_2013_derived.dta`). Identify variables related to the cohort member's weight.
...
`lookfor()` has a simple interface - you provide a `data.frame` (or `tibble`) and a search string or set of search strings. The function then identifies variables that match these strings, searching not just in the variable names, but also in variable and value labels. For instance, to find variables related to "overweight" in `mcs_cm_derived_17y`, we can enter the following:
...
Regular expressions are also used within the `stringr` package. `str_detect(label, "IOTF")` above returned a logical vector equal `TRUE` where the variable label contained "IOTF".

XX-LABELLED FUNCTIONS FOR GETTING ATTRIBUTES.

### Task II

1. Find variables related to obesity or BMI in the `mcs_cm_derived_17y` dataset. Consider that the variable label may use related but different terms.

```{r}
#| output: false
#| code-fold: true
#| code-summary: "Reveal the solution"
lookfor(mcs_cm_derived_17y, "obese|body|bmi") # Multiple terms to capture variations
```
...
1. Find variables related to height and weight using `mcs_dict_clean`. In which sweeps was weight collected from MCS parents?
...
2. Create a data dictionary for all NCDS files. In which sweeps was information on diabetes collected?
...
Note, these variables aren't helpful on their own because they need to be loaded with identifier variables to make sure the data can be ascribed to an individual. In the next step, we load the data using `read_dta()` using the `col_select` argument, and adding relevant identifier variables.
...
XX-FIND VARIABLES FOR HEIGHT, WEIGHT, AND COGNITIVE ABILITY, SEX, AND AGE IN ALL SWEEPS OF MCS.